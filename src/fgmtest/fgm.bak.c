...
//this is a small helper function that just implements the bit-counting algo we used in diffCalc(), but only on one uint32_t. this will nominally be used with diff sequences.
...
//	for(x = len - 1; x >= 0; x--)	//for all of the bins...
	...
	{	
		...
		//commentary: compound diff sequence evaluations are done so we can reliably detect indels that occur on or near boundaries of uint32_t's.
...
{
		int x = 0;
		uint32_t diff;	//the temp and final diff variable.
		unsigned int dCount = 0;
...
{	
	...
	uint32_t rs [21];
	uint32_t ds [21];
	uint32_t * refSeq = rs;//&(rs[0]);
	uint32_t * diffSeq = ds;//&(ds[0]);
