...
/* figure out how many CL platforms are available */
  ...
  fg_context = clCreateContext (NULL, fg_num_devices, fg_devices, NULL, NULL, &ret);
  errorOnNotSuccessNull(ret, context, "Failed to create context: %d", ret);

  
  /* create a command queue for the GPU device */
  fg_command_queue = clCreateCommandQueue (fg_context, fg_devices[0], 0, &ret);
  errorOnNotSuccessNull(ret, fg_command_queue, "Failed to create a command queue: %d", ret);
  ...
  fg_program = clCreateProgramWithSource (fg_context, 1, (const char **) &fg_prgm_data, &fg_prgm_size, &ret);
  errorOnNotSuccessNull(ret, fg_program, "Failed to create program with source: %d", ret);

  
  /* compile the program.. (it uses llvm or something) */
  ret = clBuildProgram (fg_program, fg_num_devices, fg_devices, NULL, NULL, NULL);
  if (CL_SUCCESS != ret)
    {
      size_t size;
      ...
      ret = clGetProgramBuildInfo (fg_program, fg_devices[0], CL_PROGRAM_BUILD_LOG,
                                   4096, log, &size);
  ...
  fg_kernel = clCreateKernel (program, "fgm", &ret);
  errorOnNotSuccessNull(ret, kernel, "Failed to create kernel: %d", ret);
  ...
  //list
  device->fg_list_buf = clCreateBuffer (device->fg_context, CL_MEM_READ_WRITE, sizeof (uint) * listSize >> 4 + 1, NULL, &ret);	//allocate the reference genome space...
  errorOnNotSuccessNull(ret, device->fg_list_buf, "Failed to create buffer: %d", ret);
  clEnqueueWriteBuffer(command_queue, device->fg_list_buf, CL_TRUE, 0, (listSize >> 4)*sizeof(uint), (void*) list, 0, NULL, NULL);	//copy list to GPU...
  
  //listLen
  ret = clSetKernelArg (kernel, 2, sizeof(cl_mem), &listLen);
...
//  /* copy the contents of fg_ret_buf from the CL device to the host (CPU) */
	ret = clEnqueueReadBuffer (device->command_queue, device->fg_res_buf, true, 0, sizeof (mData) * device->batchSize, results, 0, NULL, NULL);
...
//now we have the results on our local memory.
	...
	for(i = 0; i < device->batchSize; i++)	//print out any mutations...
	{
		for(j = 0; j < result[i]->len; j++)
			switch(result[i]->ins[j])
			{
				case DEL:
					printf("Type: DEL                    Location: %u\n", result[i]->ins[j], result[i]->locs[j]);
					break;
