...
double gpu_cgm(uint32_t* aList, uint32_t* bList, int aLength, int bLength,
...
int main(int argc, char* argv[]) {
	...
	for (i = 0; i < reps; i++) {
		...
		doubleMatchAid(aList, bList, listSize, listSize, keySize, &matches, 0,
				0);
		...
		elapsedTime = gpu_cgm(aList, bList, listSize, listSize, keySize,
				&matches2, "cgmcl.cl", cpus, 1);

		binSearch += elapsedTime;
		if(DEBUG) printf("Binary Search: %f\n", elapsedTime);

		elapsedTime = gpu_cgm(aList, bList, listSize, listSize, keySize,
				&matches, "cgmcl2.cl", listSize, listSize);

		parallel += elapsedTime;
		if(DEBUG) printf("Simple Parallel: %f\n", elapsedTime);
