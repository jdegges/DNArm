...
int main(int argc, char* argv[]) {
	...
	for (i = 0; i < reps; i++) {
		...
		elapsedTime = gpu_cgm_image(aList, bList, listSize, listSize, keySize,
				&matches, "cgmcl_image.cl", listSize, listSize);

		image += elapsedTime;
		printf("Using Images: %f\n", elapsedTime);

	}

	printf("\n*****Results*****:\n");
	printf("Sequential: %f\n", seq / reps);
	printf("Parallel w/ Images: %f\n", image / reps);
