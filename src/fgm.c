...
//		*dCount += temp;
		...
		if(smallCount(lead) < (THRESHOLD/2) || smallCount(follow) < (THRESHOLD/2))
			continue;
...
//			rVal = primeLoc + 8 * x;	//the proper location (in nucleotide places) will be here. 8*x is the offset (how much we shifted over in each loop)
//			temp = primeCount;	//we should use this value as the 
//		}
//		*dCount += temp;	//increment total diffCount
	}	
	return primeLoc;
}

unsigned int mergeCount(uint32_t * leadSeq, uint32_t * followSeq, uint32_t cPoint, int len)	//changepoint, and read seq length in uint32_t's.
{
	int i, j;
...
void mutationDetect(mData * returnData, uint32_t *  diffSeq, uint32_t * rdSeq, int rdLen, uint32_t startLoc)	//the only thing detected here will be mutations or errors
{
	int i, j;
...
//printf("9\n");		
		...
		delH = smallCount(diffSeq[len*lowest]);	//i think these are right. but im tired. look at tomorrow...
...
//			indelLoc = breakDetect(&(diffSeq[len*lowest]), &(diffSeq[len*highest]), len);	//ORDER MATTERS HERE. for del, the low index goes first.
...
//		if(smallCount(lead[0]) > THRESHOLD/2 && smallCount(follow[3]) > THRESHOLD/2)
...
//		returnData -> len = mCount;	//store the length of the mutation index for this read.
