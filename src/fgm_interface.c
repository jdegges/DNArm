...
{
	int i, x;
	mData * mutations;	//keeping track of our mutations...
	
	
	for(x = 0; x < cgmDLen; x++)	//run through all elements of our batch.
	{
		mutations = fgm(list, listLen, 48, cgmData[x].matches, cgmData[x].length, cgmData[x].read);	//try to obtain a fine grain match of the given readSequence.
		if(mutations != NULL)	//make sure there's actually data to print out...
		{
